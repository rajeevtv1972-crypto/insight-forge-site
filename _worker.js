...
<meta name="twitter:url" content="${escapeAttr(articleMeta.url)}">${breadcrumbStyles}${relatedStyles}${breadcrumbData ? '<script type="application/ld+json">' + JSON.stringify(breadcrumbData) + '</script>' : ''}`,
      ...
      .on('body', {
        element(element) {
          element.append(`<script>(function(){var navs=document.querySelectorAll('nav');navs.forEach(function(nav){var wrap=nav.querySelector('.nav-container')||nav;if(!wrap.querySelector('a[data-search-link]')){var s=document.createElement('a');s.href='search.html';s.textContent='🔍 Search';s.setAttribute('data-search-link','true');s.setAttribute('aria-label','Search Insight Forge');wrap.appendChild(s)}if(!wrap.querySelector('a[href*="faq.html"]')){var f=document.createElement('a');f.href='faq.html';f.textContent='FAQ';wrap.appendChild(f)}});document.addEventListener('click',function(ev){var a=ev.target.closest&&ev.target.closest('a[href]');if(!a||a.target==='_blank')return;try{var u=new URL(a.href,location.href);if(u.origin!==location.origin||!u.pathname.endsWith('.html'))return;if(u.pathname==='/latest-news.html'||u.pathname==='/search.html')return;var eventType=a.hasAttribute('data-search-result')?'search':'click';var body=JSON.stringify({event:eventType,url:u.pathname});if(navigator.sendBeacon){navigator.sendBeacon('/api/track',new Blob([body],{type:'application/json'}));}}catch(e){}});})();</script>`, { html: true });
        }
      })
      .transform(response);
